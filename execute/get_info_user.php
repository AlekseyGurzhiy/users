<?php
	...
	$query_name = mysqli_query($link, $query_string);
	$number = mysqli_num_rows($query_name);
	if ($number==1){
		$row = mysqli_fetch_assoc($query_name);
		$query_range = mysqli_query($link,"SELECT `name` FROM `range` WHERE `id`='".$row['id_range']."'");	
		$row_range = mysqli_fetch_assoc($query_range);

		$query_range_employ = mysqli_query($link,"SELECT `name` FROM `range_employ` WHERE `id`='".$row['id_range_employ']."'");	
